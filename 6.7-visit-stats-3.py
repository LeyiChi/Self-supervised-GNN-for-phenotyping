...
# 按照PERSON_ID和DIFF_YRS进行分组，并统计CLUSTER列表
df_yrs_status = df_s2.groupby(['PERSON_ID', 'YRS'])['TM_LABEL'].apply(list).reset_index()
...
for i in range(2,10):
    ...
    if i==2:
        sns_plot.set_xlabel("Disease status to 2nd year", fontweight='bold')
        sns_plot.set_ylabel("Disease status from 1st year", fontweight='bold')
    elif i==3:
        sns_plot.set_xlabel("Disease status to 3rd year", fontweight='bold')
        sns_plot.set_ylabel("Disease status from 2nd year", fontweight='bold')
    elif i==4:
        sns_plot.set_xlabel("Disease status to 4th year", fontweight='bold')
        sns_plot.set_ylabel("Disease status from 3rd year", fontweight='bold')
    else:
        sns_plot.set_xlabel("Disease status to " + str(i) + "th year", fontweight='bold')
        sns_plot.set_ylabel("Disease status to " + str(i-1) + "th year", fontweight='bold')
...
fig.savefig('./3.5. visit_transform_tm_2.tiff')
fig.savefig('./3.5. visit_transform_tm_2.png')
